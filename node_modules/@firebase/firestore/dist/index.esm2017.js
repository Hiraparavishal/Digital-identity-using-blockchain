...
import { getUA, isReactNative, isElectron, isIE, isUWP, isBrowserExtension } from '@firebase/util';
...
(function (MutationType) {
    ...
    MutationType[MutationType["Verify"] = 4] = "Verify";
...
class RefValue extends FieldValue {
    ...
    approximateByteSize() {
        return (this.databaseId.projectId.length +
...
class ArrayValue extends FieldValue {
    ...
    approximateByteSize() {
        return this.internalValue.reduce((totalSize, value) => totalSize + value.approximateByteSize(), 0);
    }
    toString() {
...
class LocalStore {
            ...
            promises.push(documentBuffer.getEntries(txn, updatedKeys).next(existingDocs => {
                remoteEvent.documentUpdates.forEach((key, doc) => {
                    ...
                    if (doc instanceof NoDocument &&
                        ...
                        debug(LOG_TAG$1, 'Ignoring outdated watch update for ', key, '. Current version:', existingDoc.version, ' Watch version:', doc.version);
                    }
    ...
    async synchronizeLastDocumentChangeReadTime() {
        this.lastDocumentChangeReadTime = await this.persistence.runTransaction('Synchronize last document change read time', 'readonly-idempotent', txn => this.remoteDocuments.getLastReadTime(txn));
    }
}
/**
...
class IndexedDbPersistence {
    ...
    updateClientMetadataAndTryBecomePrimary() {
        return this.simpleDb
            ...
            debug(LOG_TAG$2, 'Releasing owner lease after error during lease refresh', e);
...
class MemoryPersistence {
    ...
    static createLruPersistence(clientId, params) {
        const factory = (p) => new MemoryLruDelegate(p, params);
